...
export function TeacherCard({
  ...
  const handleLevelClick = (level) => {
    ...
    // Получаем текущие данные из localStorage для этой карточки
    const cardState =
      JSON.parse(localStorage.getItem(`teacherCard-${id}`)) || {}
    console.log('Saved cardState:', { ...cardState, storedLevel: level })

    // Сохраняем состояние для этой карточки в localStorage
    localStorage.setItem(
      `teacherCard-${id}`,
      JSON.stringify({ ...cardState, storedLevel: level })
    )
  }
